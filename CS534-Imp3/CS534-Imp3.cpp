...
pair<int, bool> decisionStump(vector<pair<int, vector<int>>> examples, vector<double> weights) {
	...
	for(int i = 0; i < examples[1].second.size(); i++){
		...
		posProbs = (((double)falseClass[0] + 1.0) / ((double)falseClass[1] + (double)falseClass[0] + 2.0)) * (((double)trueClass[1] + 1.0) / ((double)trueClass[0] + (double)trueClass[1] + 2.0));

		// calculate invProbs
		invProbs = (((double)falseClass[1] + 1.0) / ((double)falseClass[1] + (double)falseClass[0] + 2.0)) * (((double)trueClass[0] + 1.0) / ((double)trueClass[0] + (double)trueClass[1] + 2.0));
